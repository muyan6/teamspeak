  ...
  getUnlockedCount(): number {
    const row = this.db.prepare(
      `SELECT COUNT(DISTINCT g.client_database_id) as cnt
       ...
       WHERE g.server_key = ? AND u.unique_identifier NOT IN ('/4MNT/c3KE4sXuRGHedmmnFDZYc=', '+NEl0Wet9jWYFKwoBGLgV78cAzs=', 'O9VvvRMdK9B6YMDBbwi0j3L1Avs=')
    ...
    return this.db
      .prepare(
        `SELECT u.nickname as nickname, u.client_database_id as clientDatabaseId, u.unique_identifier as uniqueIdentifier,
         ...
         JOIN user_online_duration u
           ON u.server_key = g.server_key AND u.client_database_id = g.client_database_id
         WHERE g.server_key = ? AND g.level_id = ? AND u.unique_identifier NOT IN ('/4MNT/c3KE4sXuRGHedmmnFDZYc=', '+NEl0Wet9jWYFKwoBGLgV78cAzs=', 'O9VvvRMdK9B6YMDBbwi0j3L1Avs=')
           AND lower(u.nickname) NOT IN ('musicbot', 'ts3bot', 'sinusbot', 'bot', 'tsbot', 'serverquery')
    ...
    const levels = this.db
      .prepare(
        `SELECT l.id as id, l.title as title, l.hours as hours,
                COUNT(DISTINCT CASE WHEN u.unique_identifier NOT IN ('/4MNT/c3KE4sXuRGHedmmnFDZYc=', '+NEl0Wet9jWYFKwoBGLgV78cAzs=', 'O9VvvRMdK9B6YMDBbwi0j3L1Avs=') AND lower(u.nickname) NOT IN ('musicbot', 'ts3bot', 'sinusbot', 'bot', 'tsbot', 'serverquery') THEN g.client_database_id END) as unlockedCount
