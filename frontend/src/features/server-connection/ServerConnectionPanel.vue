...
<template>
  <div>
    ...
    <div v-if="connection" class="conn-status" :class="{ ok: connection.connected }">连接状态：{{ connection.connected ? '已连接' : '未连接' }}</div>
    <div class="field">
    ...
    <div class="modal-actions">
      <button class="btn primary" :disabled="saving" @click="save">{{ saving ? '保存中...' : '保存并重连' }}</button>
    </div>
  </div>
</template>
